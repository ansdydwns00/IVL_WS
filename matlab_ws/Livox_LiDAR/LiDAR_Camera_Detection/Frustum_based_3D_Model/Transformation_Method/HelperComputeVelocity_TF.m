        ...
        for i = 1:size(modelInfo.Distance,1)
            ...
            for j = 1:size(prev_detect.Distance,1)
                if strcmp(modelInfo.ID{i}, prev_detect.ID{j})
                    ...
                    prev_cuboid = [prev_detect.BoxCenter(j,:), prev_detect.BoxSize(j,:)];
                    curr_cuboid = [[prev_BoxCenter(1),prev_BoxCenter(2),prev_BoxCenter(3)],modelInfo.BoxSize(i,:)];

                    [dx,dy] = HelperComputeOrientation_TF(prev_cuboid,curr_cuboid,Cx,Cy);
